...
| **设置页** | 日常配置调整（默认模式 / 主席 / agent 数 / 语言 / prefer 顺序、启停模型）+ 轻量接入（重扫凭证、加自定义 OpenAI 兼容端点）；改动即时对下一场辩论生效。范围裁定与凭证边界见设计笔记 `docs/design-notes/web-gui-config.md` |

**明确不做**：OAuth 登录 / 设备码等冷启动全量引导（仍走 CLI `council setup`）、编辑/删除会话、模型结构性字段编辑、鉴权、多用户、远程访问。
